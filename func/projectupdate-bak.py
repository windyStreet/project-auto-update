...
class ProjectUpdate(object):
    ...
    def updateProcess(self):
        FormatPrint.printInfo("即將被更新的组为:" + str(self.willBeUpdatedGroup))
        self.willBeUpdateTomcatGroup_JSON = self.conf_JSON["projectname"][self.projectName][self.willBeUpdatedGroup]
        self.tomcatKillScriptPath = self.willBeUpdateTomcatGroup_JSON["tomcatkillscriptpath"]
        self.tomcatStartScriptPath = self.willBeUpdateTomcatGroup_JSON["tomcatstartscriptpath"]
        self.tomcatResourceUpdateScriptPath = self.willBeUpdateTomcatGroup_JSON["tomcatresourceupdatescriptpath"]

        if self.projectUpdate():  # 更新项目
            FormatPrint.printInfo("项目更新成功")
            if self.modifyNginxconf():  # 修改nginx配置文件
                FormatPrint.printInfo("修改nginx配置文件 成功")
                if NginxFunc.reloadNginx(self.willBeUpdateTomcatGroup_JSON["nginxreloadcmd"]):
                    if self.modifyRunStatus():
